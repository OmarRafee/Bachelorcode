...
class DCASE2023T2AE(BaseModel):
    ...
    def train(self, epoch):
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader)):
           ...
           # start_time = time.time()
            ...
            data_original = np.asarray(batch[0])
            n_steps = np.random.uniform(-10, 10)
            data = librosa.effects.pitch_shift(y= data_original, sr= 16000 , n_steps =n_steps)
    ...
    def eval(
      ...
      #  def apply_pitch_shift(data, sr=16000, max_shift=10):
  
     #       data_np = data.cpu().numpy()

    # Initialize an array to store shifted data
    #        shifted_data = np.zeros_like(data_np)

    # Apply pitch shifting to all samples in the data tensor
   #         for i in range(data_np.shape[0]):
  #              n_steps = np.random.uniform(-max_shift, max_shift)
 #               shifted_data[i] = librosa.effects.pitch_shift(data_np[i], sr=sr, n_steps=n_steps)

    # Convert shifted data back to a PyTorch tensor and move it to the same device as original data
#            data_shifted = torch.from_numpy(shifted_data).to(data.device).float()
        ...
        for j, batch in enumerate(test_loader):
            ...
            #data_shifted = apply_pitch_shift(data)
